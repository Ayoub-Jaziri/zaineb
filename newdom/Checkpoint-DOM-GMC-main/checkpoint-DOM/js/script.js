...
for (let i = 0; i < plus.length; i++) {
  ...
  btnMoins[i].addEventListener("click", function () {
    ...
    subtotal[i].innerHTML = Number(quantity[i].innerHTML) * Number(price[i].innerHTML);
    document.querySelector("#total").innerHTML = Number(document.querySelector("#total").innerHTML) -  Number(price[i].innerHTML) ;}
  });
